...
- タブを長時間裏に置くと rAF が止まる一方で音（AudioContext の時計）は進むので、復帰時のキャッチアップは `MAX_CATCHUP_FRAMES`（64 コマ ≒ 8 ターン）で頭打ちにしている。超過分は**8 の倍数で読み飛ばす**（拍グリッドに対する位相 `%8` を保つため）。読み飛ばしはオートマトンを進めないので、`stageInteractive` の解放・探索ヒント・オープニングのドリーは`currentFrame` ではなく**実際に踊ったコマ数 `dancedFrames`** を見る（絶対フレームの `===` 判定だと飛び越えて二度と発火しない）。上限なしだと 12 時間放置で復帰時に約 23 秒フリーズしていた
