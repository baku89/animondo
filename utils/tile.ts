...
const TILE_DISPLAY_TABLE: TileDisplay[][] = [
...
export function moveToTileDisplay(move: Move): TileDisplay {
	return (
		TILE_DISPLAY_TABLE[move.in]?.[move.out] ?? {tile: Tile.None, rotation: 0}
	)
}

// TileDisplayをUint8に変換（パックド形式）
// Format: tile (4 bits) + rotation (2 bits) in lower 6 bits
export function tileDisplayToUint8(tileDisplay: TileDisplay): number {
	return tileDisplay.tile | (tileDisplay.rotation << 4)
}
